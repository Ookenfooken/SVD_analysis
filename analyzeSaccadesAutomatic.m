function [trial, saccades] = analyzeSaccadesAutomatic(trial, currentTrial, saccades, name, trialType)
...
if strcmp(name, 'minuteSaccade')
    trial.saccades.onsets = trial.saccades.X.onsets;
    trial.saccades.offsets = trial.saccades.X.offsets;
   % cut out blinks first - use y, because we don't expect any drastic
    % velocity changes in y (movement is in x!)
    blinkIdx = ones(length(trial.saccades.onsets),1);
    ...
    trial.saccades.amplitudes = trial.eyeX_filt(trial.saccades.offsets) - trial.eyeX_filt(trial.saccades.onsets);
    trial.saccades.numBlinks = length(trial.saccades.X.onsets) - length(trial.saccades.onsets);
    trial.saccades.numSaccTotal = length(trial.saccades.onsets);
    goalIdx = ones(length(trial.saccades.onsets),1);
    for i = 1:length(trial.saccades.amplitudes)
        if abs(trial.saccades.amplitudes(i)) < 2.8
            goalIdx(i,1) = 0;
        end
    end
    trial.saccades.goalIdx = goalIdx;
    goalIdx = logical(goalIdx);
    % calculate saccade direction
    for i = 1:length(trial.saccades.onsets)
        if trial.eyeX_filt(trial.saccades.offsets(i)) < trial.eyeX_filt(trial.saccades.onsets(i))
    ...
    if ~isempty(trial.saccades.accuracies)
        trial.saccades.accuraciesGoal = trial.saccades.accuracies(goalIdx);
        trial.saccades.accuraciesCorrective = trial.saccades.accuracies(~goalIdx);
        trial.saccades.amplitudesGoal = trial.saccades.amplitudes(goalIdx);
        trial.saccades.amplitudesCorrective = trial.saccades.amplitudes(~goalIdx);
    else
        trial.saccades.accuraciesGoal = NaN;
        trial.saccades.accuraciesCorrective = NaN;
        trial.saccades.amplitudesGoal = NaN;
        trial.saccades.amplitudesCorrective = NaN;
    end
    trial.saccades.numSaccGoal = length(trial.saccades.accuraciesGoal);
    trial.saccades.numCorrective = trial.saccades.numSaccTotal - trial.saccades.numSaccGoal;
else
    %% calculate direction first
    if ~isempty(trial.saccades.X.onsets)
        for i = 1:length(trial.saccades.X.onsets)
            if trial.eyeX_filt(trial.saccades.X.onsets(i)+ 40) - trial.eyeX_filt(trial.saccades.X.onsets(i)) > 0
                trial.saccades.direction(i,1) = 1;
            else
                trial.saccades.direction(i,1) = 0;
            end
